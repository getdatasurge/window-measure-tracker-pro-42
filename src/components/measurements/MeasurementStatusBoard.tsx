...
import { useMeasurements, setupMeasurementsSubscription } from '@/hooks/useMeasurements';
...
const MeasurementStatusBoard: React.FC = () => {
  ...
  // Use our custom hook to fetch and manage measurements
  const { 
    measurements, 
    isLoading: loading, 
    error,
    invalidateMeasurements
  } = useMeasurements(filter);

  // Setup Supabase realtime subscription for immediate updates
  useEffect(() => {
    const unsubscribe = setupMeasurementsSubscription(queryClient);
    return () => {
      unsubscribe();
    };
  }, [queryClient]);
