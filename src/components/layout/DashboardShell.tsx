...
const DashboardShell: React.FC<DashboardShellProps> = ({
  ...
  return <div className="flex min-h-screen bg-[#0f0f0f] text-white">
      <DashboardSidebar collapsed={sidebarCollapsed} toggleCollapsed={toggleSidebar} />
      <div className={`flex-1 transition-all duration-300 ${sidebarCollapsed ? 'ml-16' : 'ml-64'} ${isMobile ? 'ml-0' : ''}`}>
        <div className="flex flex-col min-h-screen">
          <header className="h-16 border-b border-zinc-800/70 flex items-center px-6">
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-4">
                {isMobile && <button onClick={toggleSidebar} className="p-2 rounded-md hover:bg-zinc-800/50">
