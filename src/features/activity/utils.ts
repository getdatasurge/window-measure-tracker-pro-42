...
export const transformActivityData = (data: ActivityData[]): TeamActivity[] => {
  return data.map(item => {
    ...
    const profileName = item.profiles?.full_name || item.user?.name || 'Unknown User';
    const avatarUrl = item.profiles?.avatar_url || item.user?.avatar || `/lovable-uploads/f1ba8f91-019b-4932-9d0e-5414aef0ed47.png`;
    const projectName = item.projects?.name || item.content?.project?.name || 'Unknown Project';
    const actionType = item.action_type || item.type || 'update';
    const description = item.description || item.content?.description || 'performed an action';
    
    // Process metadata
    const { metadata, targetType } = processMetadata(item.metadata);
    
    const iconType = mapActionTypeToIcon(actionType);
    const timestamp = item.performed_at || item.timestamp || new Date().toISOString();

    return {
      id: item.id,
      user: {
        id: item.performed_by || item.user?.id || 'unknown',
        name: profileName,
        avatar: avatarUrl
      },
      timestamp,
      ...
      target: projectName,
      targetType,
      timeAgo: formatTimeDistance(timestamp),
      icon: iconType,
      metadata,
